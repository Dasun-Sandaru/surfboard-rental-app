...
RentalModel createMockRental({
  ...
  return RentalModel(
    id: 'test_return_scenario_hourly',
    shopId: 'shop1',
    customerId: 'cust1',
    itemId: 'QA_BOARD_001',
    staffId: 'staff1',
    ...
    rentType: RentType.hourly, // Explicitly hourly
    paymentStatus: PaymentStatus.paid,
    rate: 100, // Hourly rate
    amountExpected: 200, // Expected 2 hours
...
void main() {
  final file = File('rental_return_hourly_results.txt');
  ...
  group('Comprehensive Rental Return Matrix - HOURLY Scenarios', () {
    ...
    // Rental period definitions (2 hours expected)
        ...
        for (var depositEntry in depositScenarios.entries) {
          for (var upfrontEntry in upfrontScenarios.entries) {
            ...
            final String scenarioTitle =
                'SCN-H${scenarioNumber.toString().padLeft(3, '0')}: $timeName | $damageName | $depositName | $upfrontName';
            ...
            test(scenarioTitle, () {
              ...
              // 4. Assertions exactly matching the rules
              ...
              if (timeName == 'Before Due Time (Early)') {
                ...
                expect(
                  result.baseRent,
                  200,
                  reason: 'Actual duration billed as exactly 2 hours',
                );
                expect(
                  result.overdueFee,
                  0,
                  reason: 'No overdue penalty should apply within limits',
                );
                expect(result.totalCharges, 200 + damageFee);
              }
